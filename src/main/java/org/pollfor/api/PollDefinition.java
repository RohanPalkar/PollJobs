...
public class PollDefinition<T> {

    private final AsyncPollAction<T> action;

    private PollDefinition(AsyncPollAction<T> action) {
        this.action = action;
    }
    ...
    public static class AsyncPollAction<T> {
        ...
        */
        ...
        public<T> PollDefinition<T> until(Predicate<? super T> exitCriteria, Supplier<? super T> action){
